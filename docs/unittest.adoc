...
|  |   |-- foo.h                Top level "global" include
...
|      |-- foo.c                Top level "global" source file
|      |-- module/              Directory for modules within application
...
For example the `app/foo.c` and `test/test_foo.c` files will compiled together
and run as a test by the unit test runner script.
...
the host (quickly durring development) and on the target to support CI systems.
====
